...
export default async function handler(req: any, res: any) {
  ...
  try {
    const db = readDb();
    const allResults = db.job_results.filter(r => r.item_type === "scan_page_admins");
    ...
    return res.status(500).json({ error: error.message || "Failed to fetch latest admin access results" });
  }
}
